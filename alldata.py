    ...
    if choice == "Upload audio":
        ...
        if audio_file is not None:
           ...
           # st.audio(audio_file, format='audio/wav', start_time=0)
            ...
            model = load_model("C:\webUI\speech_audio.h5",compile=False)
                 ##############################################################################
            ...
            x_test = X1
            scaler = joblib.load('C:\webUI\scaler.pkl1')

            # Use the loaded scaler to transform data
            x_test = scaler.transform(x_test)
            x_test = np.expand_dims(x_test, axis=2)
            print('====================>1')
            path_checkpoint = "C:\webUI\training_2\cp.ckpt"
            model.load_weights(path_checkpoint)
            pred_test = model.predict(x_test)
            encoder_file = "C:\webUI\encoder1.npy"
