...
# Define the Mappings
emotion_combinations = {'depression':['sad','fear'],'anxiety':['fear','suprise'],'PTSD':['fear','angery'],'dementia':['confusion','disgust'],'suicidal thoughts':['sad','disgust'],'violent outburst':['angry','surprise']}
...
@st.cache
def get_title(pred, categories=CAT6):
    detected_emotion = categories[pred.argmax()]
    mental_state = get_mental_states(pred, categories)
    mental_states_str = ",".join(mental_states) if mental_states else "None"
    title = f"Detected emotion: {detected_emotion} - {pred.max() * 100:.2f}%\npotential mental_states: {mental_states_str}"
    return title
...
def main():
    ...
    if choice == "Upload audio":
        ...
        if audio_file is not None:
            ...
            ##############################################################################
            model = load_model("/home/ubuntu/PycharmProjects/pythonProject6/speech_audio.h5",compile=False)
            ...
            data_fin, sample_rate_fin = librosa.load(
                "/home/ubuntu/PycharmProjects/pythonProject6/"+str(path))
            ...
            x_test = X1
            scaler = joblib.load('/home/ubuntu/PycharmProjects/pythonProject6/scaler.pkl1')

            # Use the loaded scaler to transform data
            x_test = scaler.transform(x_test)
            x_test = np.expand_dims(x_test, axis=2)
            print('====================>1')
            path_checkpoint = "/home/ubuntu/PycharmProjects/pythonProject6/training_2/cp.ckpt"
            model.load_weights(path_checkpoint)
            pred_test = model.predict(x_test)
            encoder_file = "/home/ubuntu/PycharmProjects/pythonProject6/encoder1.npy"
